...
void doRender(GameState *game)
{
	...
	// if(game->enemies[0].alive == 0 && game->enemies[1].alive == 0 && game->enemies[2].alive == 0 && game->enemies[3].alive == 0)
	// {
...
void doRender2 (GameState *game)
{
	//Desenhando o Boss
	SDL_Rect bossRect = {game->chefe.x, game->chefe.y, game->chefe.w, game->chefe.h};
	SDL_RenderCopy(game->renderer, game->chefeImg, NULL, &bossRect);
	SDL_RenderPresent(game->renderer);
}

void bossLogic (GameState *game)
{
	int speedB = 5, wallWidth = 62;
	...
	for(i = 0; i < MAX_BULLETS; i++) if(bullets[i])
  	{
		...
		if(bullets[i]->x >= game->enemies[0].x && bullets[i]->x <= game->enemies[0].x+80 
			&& bullets[i]->y >= game->enemies[0].y && bullets[i]->y <= game->enemies[0].y+80)
	   	{
	   		game->enemies[0].alive=0;
	   	}

	   	if(bullets[i]->x >= game->enemies[1].x && bullets[i]->x <= game->enemies[1].x+80 
			&& bullets[i]->y >= game->enemies[1].y && bullets[i]->y <= game->enemies[1].y+80)
	   	{
			game->enemies[1].alive=0;
	   	}

	   	if(bullets[i]->x >= game->enemies[2].x && bullets[i]->x <= game->enemies[2].x+80 
			&& bullets[i]->y >= game->enemies[2].y && bullets[i]->y <= game->enemies[2].y+80)
	   	{
	  		game->enemies[2].alive=0;
	   	}

	   	if(bullets[i]->x >= game->enemies[3].x && bullets[i]->x <= game->enemies[3].x+80 
			&& bullets[i]->y >= game->enemies[3].y && bullets[i]->y <= game->enemies[3].y+80)
	   	{
...
int main(int argc, char *args[])
{
	...
	if (opcao == 1)
	{
		...
		while(gameRunning == 1)
		{
			...
			if(gameState.enemies[0].alive == 0 && gameState.enemies[1].alive==0 && gameState.enemies[2].alive==0 && gameState.enemies[3].alive==0)
			{
				gameRunning = 2;
			}
			//SDL_Delay(10);
