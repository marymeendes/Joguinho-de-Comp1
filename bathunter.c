...
void LoadGame(GameState *game)
{
	...
	//img hist
	surface = IMG_Load("hist1.png");
	if(surface == NULL)
	{
		printf("Can't find hist1.png!\n");
		SDL_Quit();
		exit(1);
	}

	game->hist = SDL_CreateTextureFromSurface(game->renderer, surface);
	SDL_FreeSurface(surface);
	...
	//instrucoes
	surface = IMG_Load("inst.png");
	if(surface == NULL)
	{
		printf("Can't find inst.png!\n");
		SDL_Quit();
		exit(1);
	}
...
int main(int argc, char *args[])
{
	...
	if(troca == 0)
	{
		...
		SDL_RenderCopy(gameState.renderer, gameState.hist, NULL, NULL);
		SDL_RenderPresent(gameState.renderer);
		SDL_Delay(4000);
	}
