...
int Menu(SDL_Window *window, GameState *game, int gameRunning)
{
	SDL_Surface *surface = NULL;
	int escolha;
	...
	switch(escolha)
	{
		...
		break;
		case 2 : 
			SDL_RenderCopy(game->renderer, game->instrucoes, NULL, NULL);
			SDL_RenderPresent(game->renderer);
		break;
		case 3 :
		break;
		...
		break;
		case 5 :
			SDL_RenderCopy(game->renderer, game->creditos, NULL, NULL);
			SDL_RenderPresent(game->renderer);
		break;
		default:
		break;
...
int main(int argc, char *args[])
{
 ...
 //    SDL_FreeSurface(bullet);
	...
	Menu(window, &gameState, gameRunning);
